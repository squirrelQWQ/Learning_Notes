...
视频介绍：https://www.youtube.com/watch?v=K1iu1kXkVoA

有界泛型：泛型+继承（既然泛型可以指代一类数据，那么自然可以和继承相组合，用继承关系来指定一类数据）

```java
<T>
<T extends Animal>
<T extends Animal & Serializable>	// 接口和父类，但是有格式限制，只能写一个类，且类必须先写
```
...
**总结：**总的来说泛型就是使用通配符等字母来占位（函数的参数、构造的参数等等），表示这个地方可以接收一类数据而非某一个特定的类型（比如c语言定义时是int那么使用时也只能用int），在使用时依据我们给定的实参类型来创建。

```java
//List 的ArrayList 实现
List<String> list1 = new ArrayList<>();	//这里<String>就指代了一个特定的类型
```
